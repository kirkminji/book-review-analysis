...
def train_and_save():
    ...
    DATA_PATH = os.path.join(BASE_DIR, '..', 'books_ml_dataset_v4.csv')
    ...
    # 7. Save Models and Metadata to the central models folder
    MODEL_DIR = os.path.join(BASE_DIR, '..', 'models')
    os.makedirs(MODEL_DIR, exist_ok=True)
    joblib.dump(reg_model, os.path.join(MODEL_DIR, 'best_reg_model.pkl'))
    joblib.dump(clf_model, os.path.join(MODEL_DIR, 'best_clf_model.pkl'))
    joblib.dump(scaler, os.path.join(MODEL_DIR, 'robust_scaler.pkl'))
    joblib.dump(final_feature_cols, os.path.join(MODEL_DIR, 'feature_cols.pkl'))
